...
df <- phenomin %>%
  right_join(covs_in, by = "FINNGENID") %>%   #This represent official genotyped dataset
...
c("Number of individuals, genotyped:", dim(covs_in)[1])  #These are gwas covariates

tmp <- df %>% 
  filter(!is.na(female)) %>% 
  filter(HYPTENS == 1, NRMED_MAX > 0) 

c("Number of all hypertensive individuals with medication:", dim(tmp)[1])
