...
exposures <- c(`ukb-b-2303`="BMI", `ukb-b-10787`="Height", `ukb-b-9405` = "Waist circumference",
               ...
               `ebi-a-GCST90002232`="Glucose", `ukb-d-30880_irnt`="Urate", `ukb-d-30710_irnt`="CRP", 
               `ebi-a-GCST90002244` = "HbA1C", `ieu-b-4832` = "eGFR"
            )
...
for(id in exposure_ids){
  ...
  # Add sample size
  outcome_dat$samplesize.outcome = 55733
  if(expos == "LDLc") {exposure_dat$samplesize.exposure = 440546}
  if(expos == "HDLc") {exposure_dat$samplesize.exposure = 403943}
  if(expos == "Triglycerides") {exposure_dat$samplesize.exposure = 441016}
  if(expos == "Total cholesterol") {exposure_dat$samplesize.exposure = 115078}
...
#Diagnostics for UACR - RES_HT
dat$samplesize.outcome = 55733 #Set sample size.
mr_report(dat)
...
data.table::fwrite(res_table, "data/MR_results_redo25.csv", sep=";")
...
ggsave("figs/resht_mr_r12_redo25.jpg", width=7, height=2.8)
ggsave("figs/resht_mr_r12_redo25.pdf", width=7, height=2.8)


# Cluster  - ordering was not replicated - lipid related variables had changed slightly.
# I reorder manually to match previous version.
cols <- c("CRP", "Triglycerides", "Waist circumference", "BMI", "Glucose", "LDLc", 
          "eGFR", "Alcohol", "Neuroticism", "UACR", "Urate", "HbA1C", "Height", "HDLc", 
          "Total cholesterol", "Smoking")
rows <- c("Inverse variance weighted", "Weighted median", "MR Egger", "Simple mode", "Weighted mode")
# Get indices
ord_method  <- match(rows, rownames(res_matrix)) 
ord_exposure <- match(cols, colnames(res_matrix))
...
ggsave("figs/resht_mr_r12_redo25_sameorder.jpg", width=7, height=2.8)
ggsave("figs/resht_mr_r12_redo25_sameorder.pdf", width=7, height=2.8)
