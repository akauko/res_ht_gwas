...
outcome_file_name <- "data/gwas_rs_r12/RES_HT_rs_renamed.gz"
if(!file.exists(outcome_file_name)){
  my_gwas <- fread("data/gwas_rs_r12/RES_HT_rsn.gz") %>%
    rename(SNP=rsid, se=sebeta, eaf=af_alt, effect_allele=alt, other_allele=ref) %>%
    mutate(Phenotype = "RES_HT")
  fwrite(my_gwas, "data/gwas_rs_r12/RES_HT_rs_renamed.gz", sep="\t")
}
...
for(id in exposure_ids){
  ...
  outcome_dat <- read_outcome_data("data/gwas_rs_r12/RES_HT_rs_renamed.gz", sep="\t", snps = exposure_dat$SNP)
...
outcome_dat <- read_outcome_data("data/gwas_rs_r12/RES_HT_rs_renamed.gz", sep="\t", snps = uacr$SNP)
...
ord_exposure <- rev(hclust(dist(t(res_matrix)))$order)  #Visually nicer in reversed order
...
ggsave("figs/resht_mr_r12.jpg", width=7, height=2.8)
ggsave("figs/resht_mr_r12.pdf", width=7, height=2.8)

# Hypertension as outcome

#Rename outcome columns to correspond defaults and add phenotype... would be faster with bash
outcome_file_name2 <- "data/gwas_rs_r12/HYPTENS_rs_renamed.gz"
if(!file.exists(outcome_file_name2)){
  my_gwas <- fread("data/gwas_rs_r12/HYPTENS_rsn.gz") %>%
    rename(SNP=rsid, se=sebeta, eaf=af_alt, effect_allele=alt, other_allele=ref) %>%
    mutate(Phenotype = "RES_HT")
  fwrite(my_gwas, "data/gwas_rs_r12/HYPTENS_rs_renamed.gz", sep="\t")
}
...
for(id in exposure_ids){
  ...
  outcome_dat <- read_outcome_data("data/gwas_rs_r12/HYPTENS_rs_renamed.gz", sep="\t", snps = exposure_dat$SNP)
...
ggsave("figs/hyptens_mr_r12.jpg", width=7, height=2.8)
ggsave("figs/hyptens_mr_r12.pdf", width=7, height=2.8)
